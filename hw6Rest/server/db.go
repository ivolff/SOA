...
func dbUpdatePlayer(db *sql.DB, player *Player) {
	if player.Sex != "" {
		cmd := "UPDATE players SET sex = $1 WHERE players.id = $2"
		_, err := db.Exec(cmd, player.Sex, player.ID)
		if err != nil {
			log.Fatal(err)
		}
	}
	if player.Email != "" {
		cmd := "UPDATE players SET email = $1 WHERE players.id = $2"
		_, err := db.Exec(cmd, player.Email, player.ID)
		if err != nil {
			log.Fatal(err)
		}
	}
	if player.Username != "" {
		cmd := "UPDATE players SET username = $1 WHERE players.id = $2"
		_, err := db.Exec(cmd, player.Username, player.ID)
		if err != nil {
			log.Fatal(err)
		}
	}
	if player.Avatar != "" {
		cmd := "UPDATE players SET avatar = $1 WHERE players.id = $2"
		_, err := db.Exec(cmd, player.Avatar, player.ID)
		if err != nil {
			log.Fatal(err)
		}
	}
